...
**Last completed stage:** Stage 8 (Search, Billing & Plan Limits)
**Current stage:** none in progress — ready for Stage 9
...
- [x] Backend unit tests: **199 passing** (`npm test`) — permission matrix (incl. Stage 8's `canManageWorkspaceSettings`), password verification, refresh rotation/reuse, invitation expiration/email-matching, document tree/ordering/archive/restore/IDOR/publish/unpublish/slug-collision (Stage 7), `CollaborationService` session lifecycle/awareness/eviction cleanup, `CollaborationPersistenceService` hydrate/flush/throttle/search-index-update-on-durable-write (Stage 8), `yjs-document.util` decode/serialize/replace, `VersionsService` list/inspect/create/restore/manual-snapshot-gating (Stage 8), `CommentsService`/`NotificationsService`/`AttachmentsService` (Stage 6), `PublicDocumentsService` (Stage 7), `EntitlementsService` (Stage 8: all 4 limit/feature assertions, FREE vs PRO, usage summary), `BillingService` (Stage 8: default subscription, idempotent `applyEvent` incl. the duplicate-`[null]`-identifiers case, mock-pay/downgrade)
- [x] Backend e2e tests: **109 passing** (`npm run test:e2e`, real Postgres/Redis) — full auth lifecycle, workspace/invitation flow, VIEWER security, tenant isolation, document flows A–G, 10 collaboration tests, 10 versions tests, 28 Stage 6 tests, 11 Stage 7 tests, plus 15 new Stage 8 tests covering all 16 critical scenarios: search by title, search by durable persisted content (via a real collaborative edit + flush), cross-workspace search isolation, unauthorized-user-cannot-search (404), archived documents excluded from results; new workspace defaults to FREE, plan/entitlement resolution after upgrade, non-owner cannot mutate billing, webhook idempotency for a duplicate `eventId` (plus wrong-secret rejection); FREE-below-limit succeeds, crossing the document limit rejected with the structured `PLAN_LIMIT_EXCEEDED` body, direct-API member-limit bypass prevented at accept time, PRO receives unlimited allowance, downgrade preserves all existing data while blocking new creation, 5 concurrent document-creation requests at the last available slot yield exactly one success; regression: all Stage 1–7 e2e tests remain green in the same run
- [x] Frontend tests: **74 passing** (`npm test`, vitest) — email/password validation, permission-matrix UI gating (incl. Stage 8's `canManageBilling`), invitation/document/collaboration/versions/comments/notifications/attachments/publish-unpublish/search/billing (Stage 8) API call shapes, document tree-building, text-diff algorithm, `isSafeUrl`/`sanitizeUrl` (Stage 7 XSS-defense unit tests)
...
- Interactive browser verification (clicking through the actual UI in a browser tab) has never been performed in this environment - no browser automation was available in any stage, including Stage 8 (the Claude-in-Chrome extension was not connected when attempted). Every stage instead verified its real behavior live via HTTP/socket.io-client scripts against the rebuilt Docker stack: Stage 6 exercised the full comment/mention/notification/attachment lifecycle including a real MinIO PUT/GET byte round-trip; Stage 7 published/viewed/edited/unpublished a real document end-to-end via curl against `localhost:3001`/`localhost:4000`, inspected the raw rendered HTML (confirming real content and SEO tags in the initial response, and confirming a stored `<script>` payload came back HTML-entity-escaped, never executable), and confirmed the public URL returns a true 404 after unpublish; Stage 8 rebuilt both Docker images from scratch and exercised register→create-workspace→create-document→search→mock-pay against the live rebuilt backend via curl, confirming the exact response shapes the new frontend code consumes, and confirmed Swagger correctly lists the new billing/search routes while excluding the webhook controller. Frontend build/lint/typecheck/tests passed on every stage
